...
**Ribbons use that evidence on a week, a month, or a five-hour window.** The kind used to be the gate: five-hour sessions were dropped so Codex's sliding session clock would not throw ribbons several times a day. That clock is refused by the jump-and-drop rule now, and the kind filter then left Zhipu and Kimi silent when the short quota people wait for came back — Zhipu's five-hour row reports no `resetsAt` at all, so a forty-point drop is the only evidence it can give. `celebratesReset` is a General setting, off by default. It is not a notification: no grant, no `UNUserNotificationCenter`, works in a `swift run` build. It names the account, and plays the system `Hero` sound with the ribbons (the same job CodexBar's fanfare does). It does **not** require a prior warning, and it is **not** greyed out while "Warn at" is Off. First sighting of a low figure is not a celebration. Two bars of the same account turning over together is one overlay, not two. Settings copy: [ui/settings.md](ui/settings.md).
