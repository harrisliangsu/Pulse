...
- Colour means usage, not brand (`UsageTint`, optional per-account `RingTint`). Spent colour is `SpentRingColour` (Emphasize by default, which still wins over a chosen tint). Spent comes from the **provider’s flags**, not from crossing 100%. See [ui/rings-and-surface.md](ui/rings-and-surface.md).
