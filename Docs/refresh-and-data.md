...
**One case per provider on that pass.** `UsageBatch.read` is the only place a full refresh fetches a primary account. The 1.4.3 sync kept two `async let qoderUsage` bindings, because this fork and upstream each added Qoder in a different part of `UsageStore.refresh` and git kept both. A second `case` does not compile. An `async let` pasted back into `UsageStore` is what `Scripts/fork-compat-check.sh` rejects. Add a provider in `UsageBatch`, not as a second binding beside the call.
