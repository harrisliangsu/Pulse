...
A joined sentence needs no extra space after a Chinese full stop (`。`). `appearanceSubtitle` only inserts a separator when the first half does not end in one.
...
|---|---|---|
| Panel | **Appearance** | Size, Spacing, Round ends, Appearance (dark / light / auto / glass) and Transparency while glass is on, Ring activity animation |
| Panel | **Rings and figures** | *Figures*: percentages at the side / on top, figure above the ring, show what's left, forecast. *Rings*: second limit, time until reset, time ring direction, ring colour scheme (Red alert / Gradient / Quiet), turn red at (under Red alert and Quiet), alert colour when docked |
...
| Application | **Notifications** | Warn at, when a limit comes back, when a reading stops arriving; Reset reminders (predicted and regular); Celebrations, including the optional five-hour / hourly ribbon |
...
**Ring colour scheme** lives at the foot of that group: `AppSettings.ringColourScheme`, Red alert / Gradient / Quiet, default Red alert. It owns every usage colour on the panel — rings, percent labels, usage bars, and card accents that follow `UsageTint`. **Turn red at** is nested under Red alert and Quiet (`AppSettings.warningThreshold`, 60–90%): the green→red step under Red alert, the green→grey step under Quiet, not an amber→red one. Gradient never draws red, including when spent, and shows no red-related control. Quiet never draws red either — healthy stays green; only the alarm and spent band go muted grey. 1.2.1's spent-only picker is gone; a stored Follow usage or Quiet value migrates onto Gradient or Quiet. Both pickers keep their localized title for accessibility even though the visible label is supplied by the row. [rings-and-surface.md](rings-and-surface.md)

**Reset reminders** fire *before* a time. Two independent switches, both off by default, each with a 12 / 24 / 48 hour lead. **Predicted reset** counts down only to an explicit Codex Resets `scheduled_for`; a watch with no time stays on the card and does not notify. **Regular resets** counts down to each shown window's own `resetsAt`, and skips a window shorter than the lead. Both are real notifications, so they are greyed out in a `swift run` build and ask for the same grant as the group above. They are not "When a limit comes back", and they are not the ribbons. [../notifications.md](../notifications.md)

**Celebrations** is not a notification, and it is not an advance reminder. Two switches: **Celebrate a reset** plays full-screen ribbons and the system Hero sound *after* a **week or month** turns over, with the account's name on them. **Also celebrate hourly quota resets** adds five-hour windows (Zhipu, Kimi, and anyone else who reports one). Both off by default. The hourly row is greyed out while the parent is Off. Neither goes through `UNUserNotificationCenter`, so they are not greyed out in a `swift run` build and do not ask for a grant. Not tied to "Warn at". Same unambiguous-reset evidence as the after-the-fact notification, one overlay per account per pass. A Codex session clock that only slides a few minutes is not that evidence.
