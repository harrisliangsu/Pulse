...
**Ring colour scheme** lives at the foot of that group: `AppSettings.ringColourScheme`, Red alert / Gradient / Quiet, default Red alert. It owns every usage colour on the panel — rings, percent labels, usage bars, and card accents that follow `UsageTint`. **Turn red at** is nested under Red alert only (`AppSettings.warningThreshold`, 60–90%): the green→red step, not an amber→red one. Gradient and Quiet never draw red, including when spent, and show no red-related control. 1.2.1's spent-only picker is gone; a stored Follow usage or Quiet value migrates onto Gradient or Quiet. Both pickers keep their localized title for accessibility even though the visible label is supplied by the row. [rings-and-surface.md](rings-and-surface.md)
