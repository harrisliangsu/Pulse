...
- The frame must be tall/wide enough for the **tallest card**, not just the rail (`max(DockLayout.maximum…, DetailCardLayout.maximumHeight)`). A card taller than the window is sliced flat against the edge. Codex's reset-intel block (public forecast, banked credits, source line) is part of that maximum, so the window is already tall enough before the card opens.
