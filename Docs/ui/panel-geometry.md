...
- The frame must be tall/wide enough for the **tallest card**, not just the rail (`max(DockLayout.maximum…, DetailCardLayout.maximumHeight)`). A card taller than the window is sliced flat against the edge. Codex's reset-intel block (public forecast, latest announcement, banked credits, source line) is part of that maximum, so the window is already tall enough before the card opens. The latest announcement is one line of relative time plus a type-and-clock line that may wrap; its banked explanation is a tooltip, not another row.
