...
- The frame must be tall/wide enough for the **tallest card**, not just the rail (`max(DockLayout.maximum…, DetailCardLayout.maximumHeight)`). A card taller than the window is sliced flat against the edge. Codex's reset-intel block (public forecast, latest announcement, banked credits) is part of that maximum, so the window is already tall enough before the card opens. The link to the site sits on the prediction line and does not add a row. The latest announcement is one line of relative time plus a type-and-clock line that may wrap onto a second line; the type's explanation is a tooltip on those words, not another row and not an icon.
