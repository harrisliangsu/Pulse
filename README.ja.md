...
Pulse は画面の端にすっと収まる、控えめなフローティングモニターです。各サービスが報告する残量をそのまま表示します——使うのはその製品自身のクライアント経路で、Pulse のサーバーではありません——Pulse アカウントもテレメトリーもありません。Pulse が使用率を勝手に推測することはありません。
...
- **消費ペースの予測**：現在のペースでこの上限ウィンドウを越えられるかを自動で見積もり、危険があるときは枯渇予想時刻（ETA）を表示します。
...
- **オプトインの通知**：オンにするまでオフです。上限が 75/80/90/95% を越えたとき、プロバイダが使い切ったと報告したとき、警告したウィンドウが戻ったとき、何度も続けてチェックに失敗して（パネルが古い数字を静かに表示しているとき）、そして前払い残高があなたの決めた金額を下回ったときに知らせます。それぞれ一度だけ：オンにした時点ですでに線を越えていた上限はすぐに一度だけ伝え、その後はリセットされるか悪化するまで繰り返しません。
- **Spaces にやさしい**：既定では全画面アプリの Spaces に出しゃばりません。
...
- **消費履歴**：ローカルの記録・データベース・エクスポートからトークン消費を再構成し、公開 API 料金で価格付けします。直近 7 日で開き、選んだ期間を記憶し、モデルごとのトークンと概算金額を保持します。
...
- **スクリプト可**：`Pulse --json` が最後の読み取り値——プラン、すべての上限、リセット時刻、数字がどれだけ古いか——を出力します。tmux、sketchybar、Raycast、シェルプロンプトにどうぞ。キャッシュを読むだけで取得はしないので、ポーリングしても何のコストもかかりません。
...
- **プライバシー第一**：Pulse のサーバーもアカウントもテレメトリーもありません。リクエストはすでに使っているプロバイダへ送られ（macOS のシステムプロキシ設定に従います）。
...
Pulse は各サービスが報告する数字をそのまま表示します。ローカルのトークン数から使用率を推測することはありません。経路は製品ごとに異なります（文書化されたクライアント API、エディタのログイン、ローカルの language server、貼り付けたキー）——どの行にも公開の公式クォータ API があるわけではありません。貢献者向けの詳細：[Docs/providers/README.md](Docs/providers/README.md)。
...
> [!NOTE]
> **macOS での初回起動（Gatekeeper）**：  
> Pulse は Apple Developer 証明書を持たないオープンソースプロジェクトです。初回起動時に macOS がブロックすることがあります：
> - **方法 1（GUI）**：Pulse を起動し、警告を閉じ、**システム設定 → プライバシーとセキュリティ** を開き、**このまま開く** をクリックします。
> - **方法 2（ターミナル）**：
>   ```bash
>   xattr -cr /Applications/Pulse.app
>   ```
>   *（以後の更新は内蔵の Sparkle でスムーズに行われ、毎回の確認は出ません）*。
...
- **Pulse のバックエンドはなし**：Pulse のサーバー、アカウント、テレメトリーはありません。アプリはすでに使っているプロバイダと直接通信し、独自のプロキシを挟みません。macOS のシステムプロキシ設定は引き続き適用されます。
...
- **コードと会話のプライバシー**：Pulse はあなたのソースコード、ターミナル履歴、プロンプト、LLM との会話を決して読み取りません。

---

## ソースからビルド

Pulse は重い外部依存なしに、ネイティブの Swift と SwiftUI でビルドされています。
...
# またはネイティブの macOS アプリバンドルにパッケージ
./Scripts/bundle.sh
```

ツールチェーンの設定は [Docs/build-from-source.md](Docs/build-from-source.md) を参照してください。リリースの手順：[Docs/releasing.md](Docs/releasing.md)。
