...
enum QoderCookie {
    ...
    /// A console personal token (`pt-…` or `jt-…`). It lasts for the expiry
    /// set in the Qoder console, and is sent as a bearer token to the chosen
    /// site rather than as a cookie. Nil when the stored string is a session.
...
struct QoderClient: Sendable {
    ...
    func fetch(cookie: String, site: QoderSite) async throws -> QoderSnapshot {
        // A personal token is not a cookie header. Sending it through
        // `normalize` would drop it, because it has no `name=value` pair.
        let token = QoderCookie.accessToken(in: cookie)
        let header = try token == nil ? QoderCookie.normalize(cookie) : nil
